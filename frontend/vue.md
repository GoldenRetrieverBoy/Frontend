...
**Критерий самопроверки:** Изменение state.count вызывает наблюдатель с getter; snapshot остаётся исходным числом.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

watch(() => state.count, fn) читает свойство при сборе зависимостей. watch(state.count, fn) получает уже вычисленное число. В отдельном примере изменяем count с 0 на 1 и после await nextTick проверяем один вызов fn с новым 1 и старым 0. Альтернатива — toRef(state, "count"). Несколько синхронных изменений могут объединиться, поэтому не требуем вызова на каждый инкремент.
...
**Критерий самопроверки:** Каждому из трёх сценариев назначен механизм без дублирования производного состояния.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Фильтрация списка — computed: возвращает значение и не выполняет запись в storage. Сохранение черновика — watch выбранных данных с согласованной частотой, обработкой ошибки записи и очисткой отложенного сохранения. Фокус — после изменения видимости и await nextTick либо в post-watcher. Проверяем изменение фильтра, быстрый ввод и закрытие окна; computed не отправляет запросы и не фокусирует DOM.
...
**Критерий самопроверки:** После открытия поле существует и получает фокус; ожидание DOM не трактуется как ожидание сети.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

visible.value = true планирует обновление, await nextTick() ожидает его завершения, затем читаем template ref и вызываем focus. setTimeout(0) ставит задачу браузера, но не выражает контракт завершения обновления Vue и может добавлять задержку. Проверяем повторное открытие и отсутствие поля при закрытии. nextTick не обещает загрузку async-компонента или ответа API.
...
**Критерий самопроверки:** Изменение вложенного поля черновика не меняет исходный объект; отмена не отправляет save.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

При открытии создаём локальный черновик, копируя редактируемые вложенные поля. Сохранение после валидации отправляет событие save с отдельным payload; родитель принимает изменение. Отмена уничтожает черновик. Проверяем вложенный адрес, повторное открытие и новый prop во время редактирования: политику сброса или предупреждения задаём явно, не перетираем ввод молча.
...
**Критерий самопроверки:** Контракт таблицы описывает тип строки, идентичность, payload выбора и место применения attrs.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Props: rows и columns с устойчивыми идентификаторами; scoped slot cell получает row и column. Event select передаёт id строки, родитель хранит выбор. Для нескольких корней выбираем, какие attrs идут таблице, а какие контейнеру, и предотвращаем случайное двойное применение. Проверяем сортировку, slot по умолчанию и доступное имя таблицы; слот не получает скрытые данные ребёнка автоматически.
...
**Критерий самопроверки:** После редактирования и сортировки значение остаётся у той же записи; удаление соседней строки не переносит состояние.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

При key=index прежний экземпляр строки переиспользуется для другой записи на той же позиции. Используем key=item.id, уникальный и стабильный между сортировками. Проверяем изменение порядка, вставку в начало и удаление. Генерация случайного key на каждом рендере тоже неверна: она пересоздаёт экземпляры и теряет локальное состояние.
...
**Критерий самопроверки:** Проверены поздний успех, поздняя ошибка, старый finally во время нового запроса и unmount.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Добавляем shallowRef(false) для loading, устанавливаем true в начале каждого эффекта и возвращаем ref из composable. В finally записываем false только при active. Cleanup ставит active=false до abort. В тесте удерживаем два Promise: завершение старого при ожидающем новом оставляет loading=true; завершение нового даёт его данные и false. Отдельно завершаем старый после нового и проверяем неизменность данных/ошибки; после unmount поздние исходы не меняют состояние.
...
Проверка дополнений к практике (2026-09-13): [источники и очистка watch](https://vuejs.org/guide/essentials/watchers.html), [идентичность элементов списка](https://vuejs.org/guide/essentials/list.html#maintaining-state-with-key).
