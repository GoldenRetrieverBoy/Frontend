...
**Критерий самопроверки:** [] принимается, [{id:"a"},{id:42}] сообщает индекс 1; не-массив отклоняется отдельно.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Сначала Array.isArray(raw), затем последовательно isUser для каждого элемента. При первом провале возвращаем ошибку с индексом, например { ok: false, index: 1 }; успех возвращает { ok: true, users }. Для последовательного сужения собираем User[] через push только после isUser. Проверяем null, объект вместо массива, отсутствие id и пустой массив. Типовая аннотация не выполняет эти проверки.
...
**Критерий самопроверки:** Добавление cancelled приводит к ошибке исчерпывающего обработчика до добавления новой ветки.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

В union добавляем { status: "cancelled" }. Обработчик использует switch по status и в default вызывает assertNever(result), где function assertNever(value: never): never { throw new Error("Unexpected state"); }. До case "cancelled" компилятор отклоняет этот вызов; после добавления ветки с подписью "Отменено" все варианты обработаны. Текущий label с общим return result.status сам по себе неполноту не обнаруживает.
...
**Критерий самопроверки:** Сохраняются порядок внутри групп, тип элементов и исходный массив; проверены пустой ввод и одинаковые ключи.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Для [{id:1,kind:"a"},{id:2,kind:"b"},{id:3,kind:"a"}] группа a содержит элементы 1 и 3, группа b — 2. Пустой вход даёт Map размера 0. Ключ "__proto__" — обычный ключ Map.
...
keyFn отделяет выбор ключа от формы T: можно группировать пользователей по городу, а заказы по статусу. Массивы групп новые, сами элементы сохраняют ссылки. Функция не мутирует вход; keyFn по контракту также не должен его менять.
...
**Критерий самопроверки:** На свежем литерале отдельно обнаружены лишний ключ и неверное значение; as const не принимается за runtime-заморозку.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

В palette добавляем warning: "yellow" — проверка свежего литерала против Record<"success" | "error", string> обнаруживает лишний ключ. Отдельно меняем success на 42 — число несовместимо со string. Убираем каждую ошибку независимо. Для routes тип Path остаётся объединением "/" и "/orders"; присвоение другого пути не проходит проверку типов. Для переменной, созданной ранее, проверка лишних свойств может отличаться от свежего литерала.
...
**Критерий самопроверки:** id обязателен; update с одним id допустим, а удаление поля описано отдельно от пропуска.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

type Update = { id: string; name?: string; email?: string }. Для упражнения отсутствие поля означает «не менять»; удаление задаём отдельным контрактом, например email?: string | null, где null означает удалить. JSON не сохраняет undefined как значение свойства объекта, поэтому нельзя строить удаление на нём без другого протокола. Проверяем отсутствие id, update только с id, смену email и явно разрешённое удаление.
...
**Критерий самопроверки:** Выведен тип значения после await, а не Promise; объяснена граница доверия к внешнему JSON.

<details>
<summary>Разбор практики — открой после самостоятельной попытки</summary>

Для async function fn() { return { id: "1" }; } тип Awaited<ReturnType<typeof fn>> равен { id: string }. ReturnType сначала получает Promise результата, Awaited разворачивает его. Проверяем присваивание {id:"2"} и отказ для {id:2}. Если fn объявлена как возвращающая User, это обещание типов: ответ сети всё равно валидируется во время выполнения.
...
Проверка дополнений к практике (2026-09-13): [generics](https://www.typescriptlang.org/docs/handbook/2/generics.html), [исчерпывающая проверка через never](https://www.typescriptlang.org/docs/handbook/2/narrowing.html#exhaustiveness-checking).
